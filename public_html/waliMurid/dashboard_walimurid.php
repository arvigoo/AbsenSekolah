...
// Query untuk mendapatkan kelas-kelas yang diikuti oleh murid dengan NISN tertentu
$sqlNISN = "SELECT murid_NISN FROM walimurid WHERE user_id = '$user_id'";
$resultNISN = $conn->query($sqlNISN);
?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <?php
                    // Menu 1: Daftar Kelas Murid
                    if ($resultNISN->num_rows > 0) {
                        while ($rowNISN = $resultNISN->fetch_assoc()) {
                            $nisnMurid = $rowNISN['murid_NISN'];

                            // Query untuk mendapatkan kelas-kelas yang diikuti oleh murid dengan NISN tertentu
                            $sqlKelasMurid = "SELECT DISTINCT detail_kelas.kelas_id, kelas.nama_kelas 
                                              FROM detail_kelas 
                                              INNER JOIN kelas ON detail_kelas.kelas_id = kelas.id
                                              WHERE detail_kelas.siswa_NISN = '$nisnMurid'";
                            $resultKelasMurid = $conn->query($sqlKelasMurid);

                            if ($resultKelasMurid->num_rows > 0) {
                                while ($class_row = $resultKelasMurid->fetch_assoc()) {
                                    $class_id = $class_row['kelas_id'];
