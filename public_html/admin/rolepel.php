...
// Query untuk mengambil data guru dan mata pelajaran
$sqlGuruMapel = "SELECT guru_mata_pelajaran.id, guru.id AS guru_id, guru.nama AS nama_guru, mata_pelajaran.nama AS nama_mapel, kelas.nama_kelas
                FROM guru_mata_pelajaran
                INNER JOIN guru ON guru_mata_pelajaran.guru_id = guru.id
                INNER JOIN mata_pelajaran ON guru_mata_pelajaran.mata_pelajaran_id = mata_pelajaran.id
                INNER JOIN kelas ON guru_mata_pelajaran.kelas = kelas.id
                ORDER BY guru.id, mata_pelajaran.id";
$resultGuruMapel = $conn->query($sqlGuruMapel);
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Data Guru Mata Pelajaran</h1>

                    <div class="mb-3">
                        <form action="../../backend/func/input_rolepel.php">
                    ...
                    while ($row = $resultGuruMapel->fetch_assoc()) {
                        $guru_id = $row['guru_id'];
                        $guru_nama = $row['nama_guru'];

                        if ($guru_id != $current_guru) {
                            if ($current_guru !== null) {
                                echo "</tbody></table></div></div>";
                            }

                            echo "<div class='card shadow mb-4'>";
                            echo "<div class='card-header py-3'>";
                            echo "<h6 class='m-0 font-weight-bold text-primary'>Guru $guru_nama</h6>";
                            echo "</div>";
                            echo "<div class='card-body'>";
                            echo "<div class='table-responsive'>";
                            echo "<table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'>";
                            echo "<thead>";
                            echo "<tr><th>ID</th><th>Mata Pelajaran</th><th>Kelas</th></tr>";
                            echo "</thead>";
                            echo "<tbody>";
